...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (row < [self.listArray count]) {
        ArticleCell* cell = [tableView dequeueReusableCellWithIdentifier:cellDefine];
        Article* article = [self.listArray objectAtIndex:row];
        if (cell == nil) {
            NSArray *bundle = [[NSBundle mainBundle] loadNibNamed:@"ArticleCell" owner:self options:nil];
            cell = (ArticleCell *)[bundle objectAtIndex:0];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (row == [self.listArray count]) {
        ...
        NSString* requestURL = [NSString stringWithFormat:@"%@article.show.list.do?actionType=storage&requestNum=10&page=%i",[DOITUtil getUrlBase],index];
        ...
        informationDetailView.sortText = @"硬件";
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    if ([(AppDelegate*)[[UIApplication sharedApplication] delegate] getCurrntNet]==nil) {
        ...
        listArray=[[sqlm queryInformationCache:@"Storage"] retain];
        [self.listView reloadData];
    }
    else{
