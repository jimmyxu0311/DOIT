...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (row == [self.listArray count]) {
        ...
        NSString* requestURL = [NSString stringWithFormat:@"%@article.show.list.do?actionType=cloud&requestNum=10&page=%d",[DOITUtil getUrlBase],index];
        ...
        informationDetailView.sortText = @"云计算";
...
//    self.storeQueue = [[[NSOperationQueue alloc] init] autorelease];
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    if ([(AppDelegate*)[[UIApplication sharedApplication] delegate] getCurrntNet]==nil) {
        ...
        listArray=[[sqlm queryInformationCache:@"Cloud"] retain];
        [self.listView reloadData];
    }
    else{
