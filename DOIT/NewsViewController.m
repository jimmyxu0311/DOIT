...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (row == [self.listArray count]) {
        ...
        NSString* requestURL = [NSString stringWithFormat:@"%@article.show.list.do?actionType=security&requestNum=10&page=%i",[DOITUtil getUrlBase],index];
        ...
        informationDetailView.sortText = @"渠道";
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    if ([(AppDelegate*)[[UIApplication sharedApplication] delegate] getCurrntNet]==nil) {
        ...
        listArray=[[sqlm queryInformationCache:@"Security"] retain];
        [self.listView reloadData];
    }
    else{
