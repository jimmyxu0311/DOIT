...
@implementation CreamTipListView
@synthesize bkid;
...
-(IBAction)leftClick:(id)sender{
    if (pageIndex > 1) {
        ...
        NSString* urlStr = [NSString stringWithFormat:@"%@bbs.topic.list.do?fid=%@&page=%d&requestNum=%d&digest=1",[DOITUtil getUrlBase],self.bkid,pageIndex,pageSize];
...
-(void)loadContent{
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    NSString* urlString = [NSString stringWithFormat:@"%@bbs.forum.detail.do?fid=%@&digest=1",[DOITUtil getUrlBase],self.bkid];
